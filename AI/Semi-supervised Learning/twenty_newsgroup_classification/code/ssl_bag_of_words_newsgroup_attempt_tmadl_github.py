## Cant get any of the code in the guys github to actually work...
...
SSL Experimentation:
We are going to try out the classes that Tamas Madl has created
in his semisup-learn github repository:
https://github.com/tmadl/semisup-learn/tree/master/frameworks
...
basemodel = SVC(probability=True)   # https://github.com/tmadl/semisup-learn/issues/4
basemodel.fit(X_train_tfidf, y_train)
...
predicted = basemodel.predict(test_tfidf)
...
### Import SSL framework from local filesystem
#from frameworks.SelfLearning import SelfLearningModel
from frameworks import scikitTSVM



### Create ssl algorithms X data
ssl_X = np.append(X_train, X_test)
ssl_count_vect = CountVectorizer()
ssl_X_counts = ssl_count_vect.fit_transform(ssl_X)

ssl_tfidf_transformer = TfidfTransformer()
ssl_X_tfidf = ssl_tfidf_transformer.fit_transform(ssl_X_counts)
print ssl_X_tfidf.toarray()



### Create ssl algorithms y data
ssl_y = np.append(y_train, np.array( [-1 for _ in range(y_test.shape[0])] ) )
...
#ssmodel.fit(ssl_X_tfidf, ssl_y)

#ssl_predicted = ssmodel.predict(test_tfidf)
#print np.mean(ssl_predicted == y_test)





# try the S3VM in methods...
kernel = "rbf"
model = scikitTSVM.SKTSVM(kernel=kernel)
model.fit(ssl_X_tfidf, ssl_y)
